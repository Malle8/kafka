                ...
                .transform(
                        () -> new Transformer<Integer, Integer, KeyValue<Integer, Integer>>() {
                            ...
                            public KeyValue<Integer, Integer> transform(final Integer key, final Integer value) {
                                assertThat("Key and value mus be equal", key.equals(value));
                                ...
                                Integer id = store.get(value);
                                // Only assign a new id if the value have not been observed before
                                if (id == null) {
                                    final int counterKey = 0;
                                    final Integer cnt = counter.get(counterKey);
                                    final int newId = cnt == null ? 0 : cnt;
                                    counter.put(counterKey, newId + 1);
                                    // Partitions assign ids from their own id space
                                    id = newId * partitionCount + context.partition();
                                    store.put(value, id);
                                }
                                return KeyValue.pair(value, id);
                            }
